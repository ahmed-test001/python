...
class CC_CreativePage(BasePage):
    ...
    def get_Module2_DownloadApp_link_validation(self):
        logger.info(": ##### Started TOp Hero_link_validation verification #####")
        self.url_path = ExcelUtil(tc_name="").read_from_excel("EMAILPage", 4, 2)
        parent_window = self.driver.current_window_handle
        self.wait.until(EC.element_to_be_clickable((By.XPATH, "//*[@src='http://t.info.samsungusa.com/res/samsung/d9cbd9dbb0f7d90feb73ba27ef384694.jpg']"))).click()
        all_windows = self.driver.window_handles
        child_window = [window for window in all_windows if window != parent_window][0]
        self.driver.switch_to.window(child_window)
        URL = self.driver.current_url
    ...
    def get_Top_Hero_link_validation(self):
        ...
        parent_window = self.driver.current_window_handle
        self.wait.until(EC.element_to_be_clickable((By.XPATH, "//*[@src='http://t.info.samsungusa.com/res/samsung/ec257d7cac0761873ec46b6afe39a93e.jpg']"))).click()
        all_windows = self.driver.window_handles
        child_window = [window for window in all_windows if window != parent_window][0]
        self.driver.switch_to.window(child_window)
        URL = self.driver.current_url
    ...
    def get_CC_b1_Hero_Header_lineText_validation(self):
        logger.info(": ##### Started Hero Header_line text verification #####")
        self.SL = ExcelUtil(tc_name="").read_from_excel("Generic", 4, 8).strip()
        hero_headerTxt1 = self.driver.find_element_by_xpath("(//span)[23]").text
        assert self.SL in hero_headerTxt1, "Hero Header_line text Not Matching"
        logger.info(": Validate Hero Header_line text:: " + hero_headerTxt1)
        logger.info(': #####  Verification Complete  #####\n')

    def get_CC_b2_Hero_Subcopy_validation(self):
        logger.info(": ##### Started Hero Subcopy verification #####")
        self.SL = ExcelUtil(tc_name="").read_from_excel("Generic", 6, 7).strip()
        hero_headerTxt1 = self.driver.find_element_by_xpath("(//span)[25]").text
        assert self.SL in hero_headerTxt1, "Hero Header_line text2 Not Matching"
        logger.info(": Validate Hero Header_line text:: " + hero_headerTxt1)
        logger.info(': #####  Verification Complete  #####\n')

    def get_CC_b2_Module2_Subcopy2_validation(self):
        logger.info(": ##### Started footer condition text verification #####")
        self.SL = ExcelUtil(tc_name="").read_from_excel("Generic", 7, 7).strip()
        footerTxt1 = self.driver.find_element_by_xpath("(//span)[48]").text
        # print("from excel::" + str(self.SL.encode('utf-8')))
        # print("from html::" + str(footerTxt1))
        assert self.SL in footerTxt1, "Hero Text Not Matching"
    ...
    def get_CC_a2_text_validation(self):
        ...
        # payover = self.wait.until(EC.element_to_be_clickable((By.XPATH, "//a[@_label='Pay_Over_Time_Title']"))).text
        # payover = self.wait.until(EC.element_to_be_clickable((By.XPATH, "//*[@_label='Pay_Over_Time_Copy']"))).text
        payover = self.wait.until(EC.element_to_be_clickable((By.XPATH, "//*[@alt='Get the Shop Samsung App']"))).text
    ...
    def get_CC_m1_Conditiontext1_validation(self):
        ...
        footerTxt1 = self.driver.find_element_by_xpath("(//span)[123]").text
    ...
    def get_CC_m2_Conditiontext2_validation(self):
        ...
        footerTxt2 = self.driver.find_element_by_xpath("(//span)[125]").text
