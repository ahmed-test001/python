...
class CreativePage(BasePage):
    ...
    def get_EPP_or_NonEPP_verification(self):
        ...
        if "EPP" in subjectlineTxt:
            EPP_WElement = self.wait.until(EC.element_to_be_clickable((By.XPATH,"//*[@alt='Students & Educators - Military - Government - Save@Work - Login to your Samsung Discount Program account to access your exclusive pricing below.']")))
            EPP_WElement.is_displayed()
    ...
    def get_CC_subjectLine_text_validation(self):
        ...
        self.SL = ExcelUtil(tc_name="").read_from_excel("Generic", 2, 8).strip()
        self.SL_2 = ExcelUtil(tc_name="").read_from_excel("Generic", 14, 8).strip()
        subjectlineTxt = self.driver.find_element_by_xpath("(//p[@class='MsoNormal'])[4]/span").text
    ...
    def get_CC_pre_header_text_validation(self):
        ...
        subjectlineTxt = self.driver.find_element_by_xpath("(//p[@class='MsoNormal'])[4]/span").text
        self.SL_700 = ExcelUtil(tc_name="").read_from_excel("Generic", 3, 9).strip()
        self.SL_550 = ExcelUtil(tc_name="").read_from_excel("Generic", 4, 9).strip()
        self.SL_350 = ExcelUtil(tc_name="").read_from_excel("Generic", 5, 9).strip()
        self.SL_250 = ExcelUtil(tc_name="").read_from_excel("Generic", 6, 9).strip()
    ...
    def get_CC_pre_header_link_validation(self):
        ...
        # self.PH_URL_generic = ExcelUtil(tc_name="").read_from_excel("Generic", 2, 16)
        ...
        CreativePage.dropdown_validation(self)
        self.driver.close()
    ...
    def get_Module1_link_verification(self):
        ...
        self.wait.until(EC.element_to_be_clickable((By.XPATH,"//*[@alt='Galaxy S21 5G - S21 + 5G']"))).click()
        URL = self.driver.current_url
        assert self.url_path in URL, "Web Landing Page URL is not Matching."
        logger.info(": successfully verified Web Landing page URL:\n" + URL + '\n')
    ...
    def get_Module2_link_verification(self):
        ...
        time.sleep(5)
        self.url_path = ExcelUtil(tc_name="").read_from_excel("Generic", 2, 16)
        self.wait.until(EC.element_to_be_clickable((By.XPATH,"//*[@alt='See how Galaxy S21 5G stacks up and how much instant credit you can earn.']"))).click()
        URL = self.driver.current_url
        assert self.url_path in URL, "Web Landing Page URL is not Matching."
        logger.info(": successfully verified Web Landing page URL:\n" + URL + '\n')
        with open('../TextFolder_Unique_URL/UniqueList.txt', 'w')as f: f.writelines(URL)
    ...
    def get_Module3_link_verification(self):
        ...
        self.url_path = ExcelUtil(tc_name="").read_from_excel("Generic", 2, 16)
        self.wait.until(EC.element_to_be_clickable((By.XPATH, "//*[@alt='Complete your Galaxy Ecosystem with up to $250 instant credit']"))).click()
        URL = self.driver.current_url
        assert self.url_path in URL, "Web Landing Page URL is not Matching."
        logger.info(": successfully verified Web Landing page URL:\n" + URL + '\n')
        with open('../TextFolder_Unique_URL/UniqueList.txt', 'w')as f: f.writelines(URL)
    ...
    def get_CC_m1_Conditiontext1_validation(self):
        ...
        footerTxt1 = self.driver.find_element_by_xpath("(//span)[158]").text
    ...
    def get_CC_m2_Conditiontext2_validation(self):
        ...
        footerTxt2 = self.driver.find_element_by_xpath("(//span)[160]").text
        # print(self.SL.encode('utf-8'))
    ...
    def dropdown_validation(self):
        ...
        # device = Select(self.driver.find_element_by_xpath("//*[@id='usi_manufacturerSelect']"))

        elem = self.driver.find_element_by_xpath("//Select[@id='usi_manufacturerSelect']")
        for option in elem.find_elements_by_tag_name('option'):
            print(option.text)
            # if option.text == value:
            #     break
        # device.select_by_index(1)
        # for x in device:
        #     print(x)
        ...
        # time.sleep(2)
        # self.wait.until(EC.element_to_be_clickable((By.XPATH, "(//*[@_label='Hero_Text'])[2]"))).click()
        # self.wait.until(EC.element_to_be_clickable((By.LINK_TEXT, "APPLE"))).click()
        # self.wait.until(EC.element_to_be_clickable((By.XPATH, "Apple"))).click()
        # device=self.wait.until(EC.element_to_be_clickable((By.XPATH, "//*[@aria-live='polite']"))).text
        # print("Device name::" + device)
        # select_box = browser.find_element_by_name("month")
        # WebDriverWait(driver, 20).until(EC.visibilityOfAllElementsLocatedBy(By.xpath(
        #     "//li[@class='ui-menu-item'][starts-with(@id,'ui-id-')]//span[@class='autoCompleteItem__label']")));
        # options = [x for x in device.find_elements_by_tag_name("option")]
