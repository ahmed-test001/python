...
class CreativePage(BasePage):
    ...
    def get_CC_subjectLine_text_validation(self):
        ...
        self.SL = ExcelUtil(tc_name="").read_from_excel("Copy", 7, 4).strip()
    ...
    def get_CC_pre_header_text_validation(self):
        ...
        self.PH = ExcelUtil(tc_name="").read_from_excel("Copy", 8, 4).strip()
    ...
    def get_Top_Hero_link_validation(self):
        ...
        parent_window = self.driver.current_window_handle
        self.wait.until(EC.element_to_be_clickable((By.XPATH, "//*[@id='reserve_cta']"))).click()
        all_windows = self.driver.window_handles
        child_window = [window for window in all_windows if window != parent_window][0]
        self.driver.switch_to.window(child_window)
        URL = self.driver.current_url
    ...
    def get_CC_m1_Conditiontext1_validation(self):
        ...
        self.SL = ExcelUtil(tc_name="").read_from_excel("TERMS_CONDITIONS", 2, 2)
    ...
    def get_Hero_SC_Text_validation(self):
        logger.info(": ##### Started Hero_SC_Text_validation #####")
        SC = self.driver.find_element_by_xpath("//*[@id='reserve_sc']").text
        self.SC_text = ExcelUtil(tc_name="").read_from_excel("Copy", 15, 3)
        assert self.SC_text in SC, "SC Text not Matching."
        logger.info(": successfully verified Text:" + SC + '\n')
        logger.info(': #####  Verification Complete  #####\n')

    def get_Hero_image_SC_Text_validation(self):
        logger.info(": ##### Started Hero_image SC_Text_validation #####")
        imgT = self.driver.find_element_by_xpath("//*[@id='reserve_sc_image']").get_attribute("alt")
        self.img_text = ExcelUtil(tc_name="").read_from_excel("Copy", 14, 3)
    ...
    def get_Hero_image_Text_validation(self):
        logger.info(": ##### Started Hero_image_Text_validation #####")
        imgT = self.driver.find_element_by_xpath("//*[@id='reserve_hl']").get_attribute("alt")
        self.img_text = ExcelUtil(tc_name="").read_from_excel("Copy", 11, 3)
