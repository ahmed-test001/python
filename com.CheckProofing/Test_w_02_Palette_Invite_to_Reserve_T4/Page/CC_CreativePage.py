...
class CC_CreativePage(BasePage):
    ...
    def get_CC_subjectLine_text_validation(self):
        ...
        self.SL_CatchAll1 = ExcelUtil(tc_name="").read_from_excel("Generic", 1, 8).strip()
        self.SL_CatchAll2 = ExcelUtil(tc_name="").read_from_excel("Generic", 3, 8).strip()
        subjectlineTxt = self.driver.find_element_by_xpath("(//p[@class='MsoNormal'])[4]/span").text
        try:
            assert self.SL_CatchAll1 in subjectlineTxt, "Subject Line Text Not Matching"
        except:
            assert self.SL_CatchAll2 in subjectlineTxt, "Subject Line Text Not Matching"
    ...
    def get_CC_pre_header_text_validation(self):
        ...
        logger.info(": Validated Pre-Header Text:: " + self.PH_catchall)
        logger.info(': #####  Verification Complete  #####\n')
