...
class CreativePage(BasePage):
    ...
    def get_EPP_or_NonEPP_verification(self):
        ...
        if "EPP" in subjectlineTxt:
            EPP_WElement = self.wait.until(EC.presence_of_element_located((By.XPATH,"//*[@src='http://t.info.samsungusa.com/res/samsung/39e6e2bb3929e664996a7bd380e40cd5.jpg']")))
            EPP_WElement.is_displayed()
            text_EPP=EPP_WElement.get_attribute('alt')
            # text_EPP_WElement = self.wait.until(EC.element_to_be_clickable((By.XPATH,"//*[@src='http://t.info.samsungusa.com/res/samsung/39e6e2bb3929e664996a7bd380e40cd5.jpg']"))).get_attribute('alt')
            assert EPP_WElement.is_displayed(), "Web Element not Displayed."
            logger.info(": EPP Text::  " + text_EPP)
            logger.info(": successfully verified EPP version is present.")
        else:
            logger.info(': successfully verified EPP version NOT present.')
        logger.info(': #####  Verification Complete  #####\n')

    def get_Hero_HL_verification(self):
        logger.info(": ##### Started Hero_HL verification #####")
        subjectlineTxt = self.driver.find_element_by_xpath("(//p[@class='MsoNormal'])[4]/span").text
        self.HL_N10p = ExcelUtil(tc_name="").read_from_excel("Generic1", 14, 22).rstrip()
        if "N10+" in subjectlineTxt:
            WElement = self.wait.until(EC.element_to_be_clickable((By.XPATH,"//*[@src='http://t.info.samsungusa.com/res/samsung/af62a352d63560d23e1e43fcc92d6107.jpg']")))
            WElement2 = self.wait.until(EC.element_to_be_clickable((By.XPATH,"(//a[@_label='Hero_Text'])[1]"))).text
            text_webElement=WElement.get_attribute('alt')
            # print(self.HL_N10p.encode('utf-8'))
            # print(text_webElement.encode('utf-8'))
            assert text_webElement in self.HL_N10p, "Web Element not Displayed."
            logger.info(": WebElement Text::  " + str(text_webElement.encode('utf-8')))
        else:
            logger.info(': WebElement Text NOT present.')
        logger.info(': #####  Verification Complete  #####\n')

    def get_CC_subjectLine_text_validation(self):
        logger.info(": ##### Started subjectLine_text verification #####")
        self.SL = ExcelUtil(tc_name="").read_from_excel("Generic1", 9, 3).strip()
        subjectlineTxt = self.driver.find_element_by_xpath("(//p[@class='MsoNormal'])[4]/span").text
    ...
    def get_jetStick_Module_link_verification(self):
        logger.info(": ##### Started jetStick_Module ink_validation verification #####")
        self.PH_URL_generic = ExcelUtil(tc_name="").read_from_excel("Generic", 9, 8)
        parent_window = self.driver.current_window_handle
        self.wait.until(EC.element_to_be_clickable((By.XPATH, "//*[@src='http://t.info.samsungusa.com/res/samsung/f3ef998a10b4bfe2abd9ebc037ae63fb.jpg']"))).click()
        all_windows = self.driver.window_handles
        child_window = [window for window in all_windows if window != parent_window][0]
        self.driver.switch_to.window(child_window)
        URL = self.driver.current_url
    ...
    def get_Banner_validation(self):
        ...
        logger.info(': assertion Banner_with: ' + tradein)
    ...
    def get_CC_m1_Conditiontext1_validation(self):
        ...
        self.SL2 = ExcelUtil(tc_name="").read_from_excel("TERM_CONDITION", 3, 1)
        subjectlineTxt = self.driver.find_element_by_xpath("(//p[@class='MsoNormal'])[4]/span").text
        ...
        if "Non" not in subjectlineTxt:
            ...
            # print(self.SL2.encode('utf-8'))
            assert self.SL2 in footerTxt1, "Condition_Text1 Not Matching"
