...
def segment_validation():
    ...
    with open('../OutputT/OutResult.json', 'r')as jsonfile:
        ...
        if ReadConfig.read_w43_TVDealsT2_configData('TVDataGEN_Holiday', 'utm_source') in readdata['check_list'][0]['utm_source']:
            logger.info(": utm_source matched")
        else:
            logger.info(": utm_source NOT matched")
        if ReadConfig.read_w43_TVDealsT2_configData('TVDataGEN_Holiday', 'utm_medium') in readdata['check_list'][0]['utm_medium']:
            logger.info(": utm_medium matched")
        else:
            logger.info(": utm_medium NOT matched")
        if ReadConfig.read_w43_TVDealsT2_configData('TVDataGEN_Holiday', 'cid') in readdata['check_list'][0]['cid']:
            logger.info(": cid matched")
        else:
            logger.info(": cid NOT matched")
        if ReadConfig.read_w43_TVDealsT2_configData('TVDataGEN_Holiday', 'bpid') in readdata['check_list'][0]['bpid']:
