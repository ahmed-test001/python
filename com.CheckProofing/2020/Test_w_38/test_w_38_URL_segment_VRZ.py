...
class URLSegmentVRZTest(unittest.TestCase):
    ...
    def test_segment_validation(self):
        ...
        with open('../OutputT/OutResult.json','r')as jsonfile:
            ...
            if ReadConfig.read_w38_configData('VRZ', 'offerCID') in readdata['check_list'][0]['offerCID']:
                logger.info(": offerCID matched")
            else:
                logger.info(": offerCID NOT matched")
            if ReadConfig.read_w38_configData('VRZ', 'filterCarrier') in readdata['check_list'][0]['filterCarrier']:
                logger.info(": filterCarrier matched")
            else:
                logger.info(": filterCarrier NOT matched")
            if ReadConfig.read_w38_configData('VRZ', 'utm_source') in readdata['check_list'][0]['utm_source']:
                logger.info(": utm_source matched")
            else:
                logger.info(": utm_source NOT matched")
            if ReadConfig.read_w38_configData('VRZ', 'utm_medium') in readdata['check_list'][0]['utm_medium']:
                logger.info(": utm_medium matched")
            else:
                logger.info(": utm_medium NOT matched")
            if ReadConfig.read_w38_configData('VRZ', 'utm_campaign') in readdata['check_list'][0]['utm_campaign']:
                logger.info(": utm_campaign matched")
            else:
                logger.info(": utm_campaign NOT matched")
            if ReadConfig.read_w38_configData('VRZ', 'marsLinkCategory') in readdata['check_list'][0]['marsLinkCategory']:
                logger.info(": marsLinkCategory matched")
            else:
                logger.info(": marsLinkCategory NOT matched")
            if ReadConfig.read_w38_configData('VRZ', 'MKM_RID') in readdata['check_list'][0]['MKM_RID']:
                logger.info(": MKM_RID matched")
            else:
                logger.info(": MKM_RID NOT matched")
            if ReadConfig.read_w38_configData('VRZ', 'MKM_MID') in readdata['check_list'][0]['MKM_MID']:
                logger.info(": MKM_MID matched")
            else:
                logger.info(": MKM_MID NOT matched")
            if ReadConfig.read_w38_configData('VRZ', 'cid') in readdata['check_list'][0]['cid']:
                logger.info(": cid matched")
            else:
                logger.info(": cid NOT matched")
            if ReadConfig.read_w38_configData('VRZ', 'bpid') in readdata['check_list'][0]['bpid']:
