...
def segment_validation():
    ...
    with open('../OutputT/OutResult.json', 'r')as jsonfile:
        try:
            ...
            if ReadConfig.read_w42_SoundBar_configData('SoundBarDataGEN1', 'cid') in readdata['check_list'][0]['CID']:
                logger.info(": cid matched")
            else:
                logger.info(": cid NOT matched")
            if ReadConfig.read_w42_SoundBar_configData('SoundBarDataGEN1', 'promocode') in readdata['check_list'][0]['promoCode']:
                logger.info(": promocode matched")
            else:
                logger.info(": promocode NOT matched")
            if ReadConfig.read_w42_SoundBar_configData('SoundBarDataGEN1', 'marslinkcategory') in readdata['check_list'][0]['marsLinkCategory']:
                logger.info(": marslinkcategory matched")
            else:
                logger.info(": marslinkcategory NOT matched")
            if ReadConfig.read_w42_SoundBar_configData('SoundBarDataGEN1', 'utm_source') in readdata['check_list'][0]['utm_source']:
                logger.info(": utm_source matched")
            else:
                logger.info(": utm_source NOT matched")
            if ReadConfig.read_w42_SoundBar_configData('SoundBarDataGEN1', 'utm_medium') in readdata['check_list'][0]['utm_medium']:
