...
class URLSegmentvalidationTest(unittest.TestCase):
    ...
    def test_UrlSegmentvalidation(self):
        ...
        with open('../TextFolder/TestIn_UniqueURL_List.txt')as f:
            ...
            # f = open("../OutputT/OutResult_{}.json".format(''.join(random.choice(string.digits) for i in range(3))), "w")
            # f = open("../OutputT/OutResult_{}.json".format(datetime.datetime.now().strftime("%Y_%m_%d-%I_%M_%S_%p")), "w")
            f = open("../OutputT/OutResult.json", "w")
            f.write(final_json)
            logger.info( final_json)
            f.close()
            # f = open("../OutputT/bpIdList_{}.json".format(datetime.datetime.now().strftime("%Y_%m_%d-%I_%M_%S_%p")), "w")
            f = open("../OutputT/bpIdList.json", "w")
            f.write(final_bpid)
            logger.info( final_bpid)
            f.close()
    ...
    def test_segment_validation(self):
        with open('../OutputT/OutResult.json','r')as jsonfile:
            ...
            # if ReadConfig.read_test_canvas_Page_configData('WEBPage', 'offerCID') in readdata['check_list'][0]['offerCID']:
            #     logger.info(": offerCID matched")
            # else:
            #     logger.info(": offerCID NOT matched")
            if ReadConfig.read_test_canvas_Page_configData('WEBPage', 'promoCode') in readdata['check_list'][0]['promoCode']:
                logger.info(": promoCode matched")
            else:
                logger.info(": promoCode NOT matched")
            # if ReadConfig.read_test_canvas_Page_configData('WEBPage', 'utm_source') in readdata['check_list'][0]['utm_source']:
            #     logger.info(": utm_source matched")
            # else:
            #     logger.info(": utm_source NOT matched")
            # if ReadConfig.read_test_canvas_Page_configData('WEBPage', 'utm_medium') in readdata['check_list'][0]['utm_medium']:
            #     logger.info(": utm_medium matched")
            # else:
            #     logger.info(": utm_medium NOT matched")
            # if ReadConfig.read_test_canvas_Page_configData('WEBPage', 'utm_campaign') in readdata['check_list'][0]['utm_campaign']:
            #     logger.info(": utm_campaign matched")
            # else:
            #     logger.info(": utm_campaign NOT matched")
            # if ReadConfig.read_test_canvas_Page_configData('WEBPage', 'marsLinkCategory') in readdata['check_list'][0]['marsLinkCategory']:
            #     logger.info(": marsLinkCategory matched")
            # else:
            #     logger.info(": marsLinkCategory NOT matched")
            # if ReadConfig.read_test_canvas_Page_configData('WEBPage', 'MKM_RID') in readdata['check_list'][0]['MKM_RID']:
            #     logger.info(": MKM_RID matched")
            # else:
            #     logger.info(": MKM_RID NOT matched")
            # if ReadConfig.read_test_canvas_Page_configData('WEBPage', 'MKM_MID') in readdata['check_list'][0]['MKM_MID']:
            #     logger.info(": MKM_MID matched")
            # else:
            #     logger.info(": MKM_MID NOT matched")
            if ReadConfig.read_test_canvas_Page_configData('WEBPage', 'cid') in readdata['check_list'][0]['cid']:
                logger.info(": cid matched")
            else:
                logger.info(": cid NOT matched")
            if ReadConfig.read_test_canvas_Page_configData('WEBPage', 'bpid') in readdata['check_list'][0]['bpid']:
                logger.info(": bpid matched")
            else:
                logger.info(": bpid NOT matched")
            # if ReadConfig.read_test_canvas_Page_configData('WEBPage', 'tradeIn_ModelName') in readdata['check_list'][0]['tradeIn_ModelName']:
            #     logger.info(": tradeIn_ModelName matched")
            # else:
            #     logger.info(": tradeIn_ModelName NOT matched")
        logger.info('####  TEST Complete  ####')
