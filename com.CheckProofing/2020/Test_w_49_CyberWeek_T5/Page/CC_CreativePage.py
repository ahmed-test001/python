...
class CC_CreativePage(BasePage):
    ...
    def get_CC_subjectLine_text_validation(self):
        ...
        self.SL_A = ExcelUtil(tc_name="").read_from_excel("SL_PH", 3, 8).strip()
        try:
            assert self.SL in subjectlineTxt, "Subject Line Text Not Matching"
        except:
            assert self.SL_A in subjectlineTxt, "Subject Line Text Not Matching"
    ...
    def get_Module1_SubHeadLineText(self):
        ...
        self.SH_NR = ExcelUtil(tc_name="").read_from_excel("SL_PH", 3, 13).strip()
        SubHeadLine_Txt = self.driver.find_element_by_xpath("//*[@_label='Hero_Text']").text

        if "Non" in subjectlineTxt:
            # print(self.SH_NR)
            # print(SubHeadLine_Txt)
            assert self.SH_NR in SubHeadLine_Txt, "SubHeadLine Text NOT Matching."
        # elif "NonReservers" in subjectlineTxt:
        #     print(self.SH_NR)
        #     print(SubHeadLine_Txt)
        #     assert self.SH_NR in SubHeadLine_Txt, "SubHeadLine Text NOT Matching."
        else:
