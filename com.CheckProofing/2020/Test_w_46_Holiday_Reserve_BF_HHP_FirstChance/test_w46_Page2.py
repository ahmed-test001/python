...
class W_46_CC_Module_Verify2_Test(unittest.TestCase):
    url=""
    driver = None
    url_list = []
    method_list_in_Url = []
    ...
    #
    #         if "EPP_Reservers" in self.url:
    #             logger.info(": ##Select EPP_Reserve URL PATH : " + self.url +"\n")
    ...
    #         elif "Reservers" in self.url:
    #             logger.info(": ##Select only Reserve URL PATH : " + self.url +"\n")
    #             MB_smartphone=w46_SmartphoneCreativePage(self.driver)
    #             MB_smartphone.get_EPP_or_NonEPP_verification()
    #             MB_smartphone.get_pre_header_text_validation()
    #             MB_smartphone.get_subjectLine_text_validation()
    ...
    def test_a1_W46_SubjectLine_Validation(self):
        if "EPP_Reservers" in self.url:
            logger.info(": ##Select Subject_Line : " + self.url + "\n")
            MB_smartphone = w46_SmartphoneCreativePage(self.driver)
            MB_smartphone.get_subjectLine_text_validation()

    def test_a2_W46_PreHeader_Validation(self):
        if "EPP_Reservers" in self.url:
            logger.info(": ##Select PreHeader : " + self.url + "\n")
            MB_smartphone = w46_SmartphoneCreativePage(self.driver)
            MB_smartphone.get_pre_header_link_validation()

    def test_a3_W46_Module1_Validation(self):
        if "EPP_Reservers" in self.url:
            if "MB" in self.url:
                if "MB_FOLD" in self.url:
                    MB_smartphone = w46_SmartphoneCreativePage(self.driver)
