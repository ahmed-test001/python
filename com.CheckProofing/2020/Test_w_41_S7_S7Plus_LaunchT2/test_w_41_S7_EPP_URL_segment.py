...
class URLSegment_W_41_S7_EPP_Test(unittest.TestCase):
    ...
    def test_segment_validation(self):
        ...
        with open('../OutputT/OutResult.json','r')as jsonfile:
            ...
            # if ReadConfig.read_w41_S7_S7Plus_configData('S7DataEPP', 'offerCID') in readdata['check_list'][0]['offerCID']:
            #     logger.info(": offerCID matched")
            # else:
            #     logger.info(": offerCID NOT matched")
            if ReadConfig.read_w41_S7_S7Plus_configData('S7DataEPP', 'promoCode') in readdata['check_list'][0]['promoCode']:
                logger.info(": promoCode matched")
            else:
                logger.info(": promoCode NOT matched")
            # if ReadConfig.read_w41_S7_S7Plus_configData('S7DataEPP', 'skipOffer') in readdata['check_list'][0]['skipOffer']:
            #     logger.info(": skipOffer matched")
            # else:
            #     logger.info(": skipOffer NOT matched")
            # if ReadConfig.read_w41_S7_S7Plus_configData('S7DataEPP', 'utm_source') in readdata['check_list'][0]['utm_source']:
            #     logger.info(": utm_source matched")
            # else:
            #     logger.info(": utm_source NOT matched")
            if ReadConfig.read_w41_S7_S7Plus_configData('S7DataEPP', 'utm_medium') in readdata['check_list'][0]['utm_medium']:
                logger.info(": utm_medium matched")
            else:
                logger.info(": utm_medium NOT matched")
            if ReadConfig.read_w41_S7_S7Plus_configData('S7DataEPP', 'utm_campaign') in readdata['check_list'][0]['utm_campaign']:
                logger.info(": utm_campaign matched")
            else:
                logger.info(": utm_campaign NOT matched")
            if ReadConfig.read_w41_S7_S7Plus_configData('S7DataEPP', 'marsLinkCategory') in readdata['check_list'][0]['marsLinkCategory']:
                logger.info(": marsLinkCategory matched")
            else:
                logger.info(": marsLinkCategory NOT matched")
            # if ReadConfig.read_w41_S7_S7Plus_configData('S7DataEPP', 'MKM_RID') in readdata['check_list'][0]['MKM_RID']:
            #     logger.info(": MKM_RID matched")
            # else:
            #     logger.info(": MKM_RID NOT matched")
            # if ReadConfig.read_w41_S7_S7Plus_configData('S7DataEPP', 'MKM_MID') in readdata['check_list'][0]['MKM_MID']:
            #     logger.info(": MKM_MID matched")
            # else:
            #     logger.info(": MKM_MID NOT matched")
            if ReadConfig.read_w41_S7_S7Plus_configData('S7DataEPP', 'cid') in readdata['check_list'][0]['cid']:
                logger.info(": cid matched")
            else:
                logger.info(": cid NOT matched")
            if ReadConfig.read_w41_S7_S7Plus_configData('S7DataEPP', 'bpid') in readdata['check_list'][0]['bpid']:
