...
class URLSegment_W_42_TV_Deals_GEN_SoundBarQ900T_Test(unittest.TestCase):
    ...
    def test_UrlSegmentvalidation(self):
        ...
        with open('../TextFolder/TestIn_UniqueURL_List.txt')as f:
            ...
            for url in urls:
                ...
                if ReadConfig.read_w42_TVDeals_configData('WEBLink', 'soundbarq900t') in url:
                    try:
    ...
    def test_segment_validation(self):
        ...
        with open('../OutputT/OutResult.json', 'r')as jsonfile:
            ...
            # if ReadConfig.read_w42_TVDeals_configData('TVDataGEN', 'utm_campaign') in readdata['check_list'][0]['utm_campaign']:
            #     logger.info(": utm_campaign matched")
            # else:
            #     logger.info(": utm_campaign NOT matched")
            # if ReadConfig.read_w42_TVDeals_configData('TVDataGEN', 'marsLinkCategory') in readdata['check_list'][0]['marsLinkCategory']:
            ...
            if ReadConfig.read_w42_TVDeals_configData('TVDataGEN', 'cid') in readdata['check_list'][0]['cid']:
                logger.info(": cid matched")
            else:
                logger.info(": cid NOT matched")
            if ReadConfig.read_w42_TVDeals_configData('TVDataGEN', 'bpid') in readdata['check_list'][0]['bpid']:
