...
class CC_CreativePage(BasePage):
    ...
    def get_CC_subjectLine_text_validation(self):
        ...
        self.SL = ExcelUtil(tc_name="").read_from_excel("SL_PH1", 1, 4).strip()
    ...
    def get_CC_pre_header_text_validation(self):
        ...
        self.PH_Galaxy_R = ExcelUtil(tc_name="").read_from_excel("SL_PH1", 1, 5).strip()
        self.PH_Galaxy_NR = ExcelUtil(tc_name="").read_from_excel("SL_PH1", 1, 7).strip()
        self.PH_TV = ExcelUtil(tc_name="").read_from_excel("SL_PH1", 2, 5).strip()
        self.PH_TABLET = ExcelUtil(tc_name="").read_from_excel("SL_PH1", 3, 5).strip()
        self.PH_WEAR = ExcelUtil(tc_name="").read_from_excel("SL_PH1", 4, 5).strip()
        self.PH_COMPUTER_R = ExcelUtil(tc_name="").read_from_excel("SL_PH1", 5, 7).strip()
        self.PH_COMPUTER_NR = ExcelUtil(tc_name="").read_from_excel("SL_PH1", 5, 7).strip()
        self.PH_HA = ExcelUtil(tc_name="").read_from_excel("SL_PH1", 6, 5).strip()
        if "MB" in subjectlineTxt:
            if "GALAXY" in subjectlineTxt:
                if "Non" not in subjectlineTxt:
                    assert self.PH_Galaxy_R.encode('utf-8') in pheaderTxt, "Pre Header Text Not Matching"
                else:
                    assert self.PH_Galaxy_NR.encode('utf-8') in pheaderTxt, "Pre Header Text Not Matching"
            elif "TABLET" in subjectlineTxt:
                assert self.PH_TABLET.encode('utf-8') in pheaderTxt, "Pre Header Text Not Matching"
            elif "WEAR" in subjectlineTxt:
                assert self.PH_WEAR.encode('utf-8') in pheaderTxt, "Pre Header Text Not Matching"
        elif "CE" in subjectlineTxt:
            if "Non" not in subjectlineTxt:
                assert self.PH_COMPUTER_R.encode('utf-8') in pheaderTxt, "Pre Header Text Not Matching"
            else:
                assert self.PH_COMPUTER_NR.encode('utf-8') in pheaderTxt, "Pre Header Text Not Matching"
        elif "HA" in subjectlineTxt:
            assert self.PH_HA.encode('utf-8') in pheaderTxt, "Pre Header Text Not Matching"
        elif "TV" in subjectlineTxt:
            assert self.PH_TV.encode('utf-8') in pheaderTxt, "Pre Header Text Not Matching"
    ...
    def get_Module1_SubHeadLineText(self):
        logger.info(": ##### Started Text verification #####")
        self.SH = ExcelUtil(tc_name="").read_from_excel("SL_PH", 4, 8).strip()
        # self.Non_Reserve = ExcelUtil(tc_name="").read_from_excel("SL_PH", 5, 13).strip()
        # subjectlineTxt = self.driver.find_element_by_xpath("(//p[@class='MsoNormal'])[4]/span").text

        SubHeadLine_Txt = self.driver.find_element_by_xpath("//*[@id='_x0000_i1036']").text.encode('utf-8')
        print(self.SH)
        print(SubHeadLine_Txt)
        assert self.Reserve.encode('utf-8') in SubHeadLine_Txt, "SubHeadLine Text NOT Matching."
        logger.info(": Validated Module-1 SubHeadLine Text:: " + self.SH)
        logger.info(': #####  Verification Complete  #####\n')
