...
class W47_remail_CC_CreativePage(BasePage):
    ...
    def get_EPP_or_NonEPP_verification(self):
            ...
            if "EPP" in subjectlineTxt:
                EPP_WElement = self.wait.until(EC.element_to_be_clickable((By.XPATH,"//*[@src='http://t.info.samsungusa.com/res/samsung/884ca6bb27194595642c98ab3cc679ee.jpg']")))
    ...
    def get_CC_subjectLine_text_validation(self):
        ...
        self.SL_E = ExcelUtil(tc_name="").read_from_excel("SL_PH", 6, 8)
        self.SL = ExcelUtil(tc_name="").read_from_excel("SL_PH", 5, 8)
        subjectlineTxt = self.driver.find_element_by_xpath("(//p[@class='MsoNormal'])[4]/span").text
        if "MB" in subjectlineTxt:
            # print(self.SL_E)
            # print(subjectlineTxt)
            # assert self.SL_E in subjectlineTxt, "Subject Line Text Not Matching"
            assert ReadConfig.read_w46_HolidayDeals_T2_configData('proof_PH','CC_SL_N_EPP') in subjectlineTxt, "Subject Line Text Not Matching"
        elif "MB" not in subjectlineTxt:
            # print(self.SL)
            # print(subjectlineTxt)
    ...
    def get_CC_pre_header_text_validation(self):
        ...
        self.SL_E = ExcelUtil(tc_name="").read_from_excel("SL_PH", 6, 9)
        self.SL = ExcelUtil(tc_name="").read_from_excel("SL_PH", 5, 9)

        if "MB" in subjectlineTxt:
            print(ReadConfig.read_w46_HolidayDeals_T2_configData('proof_PH','cc_epp_PH'))
            print(pheaderTxt)
            # pass
            # assert "Skip the lines and save sitewide on your favorite gift-worthy products while supplies last. " in subjectlineTxt, "Pre Header Text Not Matching"
            assert ReadConfig.read_w46_HolidayDeals_T2_configData('proof_PH','cc_Nepp_PH') in pheaderTxt, "Pre Header Text Not Matching"
        else:
        # elif "EPP" not in subjectlineTxt:
        #     print(ReadConfig.read_w46_HolidayDeals_T2_configData('proof_PH', 'cc_Nepp_PH'))
        #     print(subjectlineTxt)
            # pass
            # assert "Skip the lines and get up to 80% off Galaxy smartphones and $50 or more instant credit." in subjectlineTxt, "Pre Header Text Not Matching"
            assert ReadConfig.read_w46_HolidayDeals_T2_configData('proof_PH','cc_epp_PH') in pheaderTxt, "Pre Header Text Not Matching"
